...
def main(args: argparse.Namespace):
    ...
    hmean = 2*(precious*recall)/(precious+recall)
    print(f'result : pricious-{precious}, recall-{recall}, hmean-{hmean}\n')
    print(f'feature : {args.dataset_feature}')

if __name__ == '__main__':
    logger = logging.getLogger()

    parser = argparse.ArgumentParser(description='Model test')
    parser.add_argument('-m', '--model', default='saved/CRNN_mish_adamW_0.6086.pth.tar', type=pathlib.Path, required=False, help='path to model')
    parser.add_argument('-o', '--output_dir', default='output/test_CRNN_0.6086', type=pathlib.Path, help='output dir for drawn images')
    parser.add_argument('-d', '--data_root', default='datasets', type=pathlib.Path, required=False, help='dir for input image')
    parser.add_argument('-f', '--dataset_feature', default='2017', type=str, help='dataset feature')
    ...
    parser.add_argument('-s', '--save_data', default=False, type=bool, help='determine save data')
